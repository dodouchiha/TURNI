...
st.sidebar.header("\U0001f468\u200d⚕️ Gestione Medici")
...
oggi = datetime.today()
mese = st.selectbox("\U0001f4c5 Seleziona mese", list(range(1, 13)), index=oggi.month - 1, format_func=lambda x: calendar.month_name[x])
anno = st.selectbox("\U0001f4c6 Seleziona anno", list(range(oggi.year - 1, oggi.year + 3)), index=1)
...
for medico in medici:
    df[medico] = "Nessuna"
    st.markdown(f"### \U0001f4cb Assenze per {medico}")
...
st.subheader(f"\U0001f4c5 Calendario {nome_mese} {anno}")
...
st.download_button("\U0001f4c5 Scarica Excel", data=to_excel(df), file_name=f"turni_{anno}_{mese:02d}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
